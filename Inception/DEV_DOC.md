...
- **`.env`** (`srcs/.env`) — non-sensitive configuration: domain name,
  database name, usernames, WordPress admin details, and the `DATA_PATH`
  variable used for the host-side volume location.
...
Before your first build, create the secret files:
...
echo -n "your_wp_credentials"   > secrets/credentials.txt
...
`wordpress/tools/entrypoint.sh`, `nginx/tools/entrypoint.sh`) ends with an `exec`
call to the actual server process (`mysqld`, `php-fpm8.2 -F`, `nginx -g "daemon off;"`),
so that process — not the shell script — becomes PID 1 inside its container. This is required so
...
| `wordpress_data` | `${DATA_PATH}/wordpress` | WordPress core files, uploads, themes, plugins (`/var/www/wordpress` inside the container) |
