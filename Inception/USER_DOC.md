...
| `secrets/credentials.txt` | WordPress administrator password secret |

The WordPress site's own administrator login (used at `/wp-admin`) is
configured through `srcs/.env` (`WORDPRESS_ADMIN_USER`, ensuring it does not
contain `admin` or `administrator`), and reads its password from
`secrets/credentials.txt` via Docker secrets (or `WORDPRESS_ADMIN_PASS`).
